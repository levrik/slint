...
impl Font {
    pub fn height(&self) -> f32 {
        self.fonts
            .iter()
            .map(|f| f.height() * self.pixel_size / f.units_per_em)
            .reduce(f32::max)
    ...
    pub fn text_size(
        ...
        let (longest_line_width, num_lines) = layout.text_size(text, max_width);
        euclid::size2(longest_line_width, num_lines as f32 * self.height())
    }
}
...
impl Font {
    fn with_single_face<T>(&self, mut callback: impl FnMut(&rustybuzz::Face) -> T) -> T {
        let fontdb_id = self.fonts.first().unwrap().fontdb_face_id;
        FONT_CACHE.with(|cache| {
            cache
                .borrow()
                .available_fonts
                .with_face_data(fontdb_id, |face_data, face_index| {
...
impl TextShaper for Font {
    ...
    fn shape_text<GlyphStorage: core::iter::Extend<Glyph<f32, PlatformGlyph>>>(
        &self,
        text: &str,
        glyphs: &mut GlyphStorage,
    ) {
        self.with_single_face(|face| {
                ...
                glyph_buffer.glyph_infos().iter().zip(glyph_buffer.glyph_positions().iter()).map(
                    |(info, position)| {
                        ...
                        out_glyph.platform_glyph = PlatformGlyph {
                            font_id: Some(self.fonts.first().unwrap().femtovg_font_id),
    ...
    fn glyph_for_char(&self, ch: char) -> Option<Glyph<f32, PlatformGlyph>> {
        self.with_single_face(|face| {
            let scale = self.pixel_size / (face.units_per_em() as f32);
...
impl i_slint_core::textlayout::FontMetrics<f32> for Font {
    fn ascent(&self) -> f32 {
        self.fonts
            .iter()
            .map(|f| f.ascent * self.pixel_size / f.units_per_em)
            .reduce(f32::max)
            .unwrap_or_default()
    }
    ...
    fn descent(&self) -> f32 {
        self.fonts
            .iter()
            .map(|f| f.descent * self.pixel_size / f.units_per_em)
            .reduce(f32::min)
            .unwrap_or_default()
    }
}
